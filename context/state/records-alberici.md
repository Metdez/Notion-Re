...
- 2026-06-13 audit #3 (notion-audit skill, full 3a–3e pass): **0 fills** — all sourced data (vs Alb.md) confirmed present. Full live-fetch: company record, all 11 divisions, all 13 projects, all events/memberships/locations/software. ⚠ **Membership dup issue discovered** — a prior session (06-12) added 5 net-new memberships (LCI, WCDA, CIRT, ABC, AGC-MO) plus created multiple duplicate rows for each: LCI ×3 (`8133`, `810e`, `8136`), CIRT ×3 (`8195`, `8127`, `81e6`), WCDA ×3 (`8157`, `813f`, `81fe`). All new memberships confirmed sourced and Companies-linked; duplicates flagged for **Zack UI cleanup**. Company record fully populated (17 Country values; 51 Construction Projects; 300+ People; 13 Subsidiaries; Address place set; body=Snapshot+Sources). 3a ✓ (all relation edges intact) · 3b ✓ (all division/project bodies at dossier depth) · 3c ✓ (company HQ + all 11 division Adress places set; project Adress genuinely sourceless) · 3d — memberships now 9 unique sourced rows (NSC, AGC, DBIA, USGBC, LCI, WCDA, CIRT, ABC, AGC-MO) + duplicates flagged · 3e ✓ (all 4 events tagged correctly). Genuinely sourceless (unchanged): AGC TechCon Place; Procore Groundbreak Place; project-site lat/lng; 4 project contract values (Toronto Pearson, OU Life Sciences, Williams Crossing, Wright City).
