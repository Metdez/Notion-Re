# Company Research Prompt — plug-and-play (deep research)

> Reverse-engineered from the **KBE** "Company Map" profile (Market Research → Zack Folder → Companies research).
> **Swap** `{{COMPANY}}` everywhere (and optional `{{COMPANY_WEBSITE}}` / `{{COMPANY_HQ}}`) and run in Perplexity / Claude / any deep-research model.
> **Excludes** LinkedIn Posts and People (collected separately, per Zack).
> Output = one cited JSON object (schema below) that maps 1:1 to Zack's Notion databases.
> Every fact carries an inline source URL. Dedup against existing Notion records before creating.
...
Field names map to Notion. Use `null` for unknowns and log them in `gaps`. Dates ISO `YYYY-MM-DD`. Money = USD millions, number only (`50` = $50M). Every value object carries a `source_url`.
...
```json
{
  ...
  "locations": [                           // -> Locations DB (office footprint)
    { "name": "", "address": { "value": "", "lat": null, "lng": null }, "source_url": "" }
  ],
  ...
  "memberships": [                         // -> Memberships DB (industry associations / trade orgs)
    { "association": "", "level": "", "url": "", "source_url": "" }
  ],
